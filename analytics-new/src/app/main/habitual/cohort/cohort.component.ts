...
@Component({
    ...
    styleUrls:  ['habitual.component.scss']*/
...
export class CohortComponent{
    ...
    changeOfGranularity($event) {
        ...
        this.chartFilterData.startTime = this.cohortService.getStartTime(this.chartFilterData);
        this.chartFilterData.endTime = this.cohortService.getEndTime(this.chartFilterData);
        this.setEntity();
    ...
    onFilterChanges(eventFilterData, selectedChart) {
        ...
        this.newSubscription = this.cohortService.getChartDataFromGraphQl(graphQlObject).subscribe(response => {
        ...
        if (response && response.status !== 500 && response.status !== 0) {
            try {
                ...
                if (allData && allData.error === 'nodatafound') {
                    ...
                    const newData = this.cohortService.changeChartFormatGQL(allData, parseInt(this.chartFilterData.selectedChart), this.chartFilterData.userType);
